...
public class MainActivity extends AppCompatActivity {
    ...
    String BITCOINAPI_URL="https://apiv2.bitcoinaverage.com/indices/global/ticker/BTCUSD";
    TextView textView;
    TextView textView2;
    Button getBitCoinPriceButton;
    String API_ID="NDYxNjk1YjM2NTFiNGMzYzljNTc2YTcyMjBmMzBiYzQ";
    TextView textView3;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView3=findViewById(R.id.getbitcoinprice);
        //BitCoinPrice
        getBitCoinPriceButton=findViewById(R.id.getBitCoinPrice);
        getBitCoinPriceButton.setOnClickListener(v -> {
            Log.d(TAG,"GetBitCoinPrice button pressed:)");
            AsyncHttpClient client=new AsyncHttpClient();
            RequestParams requestParams=new RequestParams();
            client.addHeader("x-ba-key", API_ID);
            getBitCoinPrice(requestParams);
        });
    }
    public void getBitCoinPrice(RequestParams requestParams){
        AsyncHttpClient client=new AsyncHttpClient();
        client.get(BITCOINAPI_URL,requestParams,new JsonHttpResponseHandler(){
            @Override
            public void onSuccess(int statusCode, Header[] headers, JSONArray response) {
                super.onSuccess(statusCode, headers, response);
                Log.d(TAG,"Http request to Bitcoin API SUCCESS:)");
                Log.d(TAG,response.toString());
            }

            @Override
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                Log.d(TAG,"HTTP request to BITCOIN API SUCCEEDED");
                Log.d(TAG,response.toString());
                BitCoinModel.getBitcoinAskingPrice(response);
                double price=BitCoinModel.price;
            ...
            public void onFailure(int statusCode, Header[] headers, Throwable throwable, JSONObject errorResponse) {
                ...
                Log.d(TAG,"Http request to Bitcoin API FAILED:(");
                Log.d(TAG,errorResponse.toString());
            }

            @Override
            public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                Log.d(TAG,"onFailure method received a call back");
